...
public class PieSettings extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstance) {
        ...
        addPreferencesFromResource(R.xml.pixeldust_settings_pie);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (preference == mBattery) {
            Settings.Secure.putIntForUser(resolver, Settings.Secure.PIE_BATTERY_MODE, value, UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mTheme) {
            Settings.Secure.putIntForUser(resolver, Settings.Secure.PIE_THEME_MODE, value, UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mStatus) {
            Settings.Secure.putIntForUser(resolver, Settings.Secure.PIE_STATUS_INDICATOR, value, UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mPieGravity) {
            Settings.Secure.putIntForUser(resolver, Settings.Secure.PIE_GRAVITY, value, UserHandle.USER_CURRENT);
            return true;
        }
        return false;
    }
